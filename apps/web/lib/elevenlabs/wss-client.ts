...
export async function connectElevenLabsWss(
  ...
  ws.addEventListener("message", (event) => {
    ...
    for (const cb of subscribers) {
      try {
        ...
        LogBus.warn("elevenlabs", "frame subscriber threw", {
  ...
  ws.addEventListener("error", () => {
    LogBus.warn("elevenlabs", "wss error event");
    for (const cb of subscribers) {
      try {
  ...
  ws.addEventListener("close", (event) => {
    closed = true;
    LogBus.debug("elevenlabs", "wss closed", {
      code: event.code,
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    ws.addEventListener(
      ...
      () => {
        ...
        LogBus.info("elevenlabs", "wss connected", { url: safeUrl });
        resolve();
      },
      { once: true },
    );
...
function parseFrame(raw: unknown): ElevenLabsFrame | null {
  if (typeof raw !== "string") {
    LogBus.debug("elevenlabs", "non-string ws frame", {
      type: typeof raw,
    });
    return null;
  }
  ...
  try {
    ...
    LogBus.warn("elevenlabs", "non-JSON ws frame", {
