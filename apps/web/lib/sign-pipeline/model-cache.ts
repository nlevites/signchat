...
async function openDb(): Promise<IDBDatabase | null> {
  ...
  return new Promise((resolve) => {
    ...
    req.onerror = () => {
      LogBus.warn("model-cache", "IndexedDB open failed", { err: req.error?.message });
      resolve(null);
    };
  });
}
...
async function idbGet(url: string): Promise<CachedModelEntry | null> {
  ...
  return new Promise((resolve) => {
    ...
    req.onerror = () => {
      LogBus.warn("model-cache", "idbGet failed", { err: req.error?.message });
      resolve(null);
    };
  });
}
...
async function idbPut(entry: CachedModelEntry): Promise<void> {
  ...
  return new Promise((resolve) => {
    ...
    tx.onerror = () => {
      LogBus.warn("model-cache", "idbPut failed", { err: tx.error?.message });
      resolve();
    };
  });
}
...
export async function fetchModelCached(
  ...
  if (!options.bypassCache) {
    ...
    if (cached) {
      LogBus.info("model-cache", `cache hit ${url}`, {
    ...
    LogBus.info("model-cache", `cache miss ${url}`);
  } else {
    LogBus.info("model-cache", `cache bypass ${url}`);
  }
  ...
  LogBus.info("model-cache", `cached ${url}`, { bytes: bytes.byteLength, hash });
...
export async function clearModelCache(): Promise<void> {
  ...
  return new Promise((resolve) => {
    ...
    store.clear();
    tx.oncomplete = () => {
      LogBus.info("model-cache", "cache cleared");
      resolve();
    };
    tx.onerror = () => resolve();
  });
