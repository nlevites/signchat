...
export async function loadOrt(): Promise<OrtModule> {
  ...
  LogBus.info("ort", `loading onnxruntime-web@${ORT_VERSION} from CDN`);
  ...
  ortPromise = dynamicImport(ORT_CDN_URL).then((mod) => {
    ...
    LogBus.info("ort", "onnxruntime-web ready", {
