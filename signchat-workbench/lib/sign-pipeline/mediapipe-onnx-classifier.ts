...
export interface MediaPipeOnnxClassifierOptions {
  ...
  onFrame?: (frame: import("./mediapipe-runner").VisionFrame) => void;
...
export class MediaPipeOnnxClassifier implements Classifier {
  ...
  async start(): Promise<void> {
    ...
    try {
      LogBus.info("classifier", "starting MediaPipeOnnxClassifier", { config: this.cfg });
      ...
      LogBus.error("classifier", "start failed", { error: error.message });
  ...
  async stop(): Promise<void> {
    ...
    LogBus.info("classifier", "stopping MediaPipeOnnxClassifier");
    await this.cleanup();
    this.transition("stopped");
  }
  ...
  private startFrameLoop(): void {
    const onFrame = (now: number): void => {
      ...
      try {
        ...
        LogBus.warn("classifier", "frame loop error", { err: String(err) });
      }
      this.queueNextFrame();
    };
    ...
    try {
      ...
      for (const cb of this.resultListeners) {
        try {
          ...
          LogBus.warn("classifier", "result listener threw", { err: String(err) });
        }
      }
    } catch (err) {
      LogBus.error("classifier", "inference failed", { err: String(err) });
    } finally {
  ...
  private installVisibilityHandler(): void {
    ...
    const handler = () => {
      ...
      this.paused = hidden;
      LogBus.debug("classifier", `tab visibility ${hidden ? "hidden" : "visible"}`, {
        paused: this.paused,
      });
    };
  ...
  private async cleanup(): Promise<void> {
    ...
    if (this.session && typeof this.session.release === "function") {
      try {
        ...
        LogBus.warn("classifier", "session.release threw", { err: String(err) });
      }
    }
    this.session = null;
    if (this.video) {
