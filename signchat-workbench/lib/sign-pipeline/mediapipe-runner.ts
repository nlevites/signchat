 ...
 * encode/decode.
 *
 * The architecture mentions GestureRecognizer; we use HandLandmarker because
 * the workbench only needs landmarks (no gesture classification), and the
 * landmark output shape is identical between the two.
 *
...
export async function createVisionRunner(
  ...
  LogBus.info("mp", `loading FilesetResolver from CDN`, { wasmBase: WASM_BASE });
  const fileset = await FilesetResolver.forVisionTasks(WASM_BASE);

  LogBus.info("mp", "creating face / hand / pose landmarkers");
  ...
  LogBus.info("mp", `landmarkers ready (${Math.round(performance.now() - startedAt)}ms)`, {
    delegate,
    numHands,
  });
  ...
  return {
    detect(video, ts) {
      ...
      try {
        ...
        LogBus.error("mp", "detectForVideo threw", { err: String(err) });
    ...
    close() {
      try {
        ...
        LogBus.warn("mp", "close failed", { err: String(err) });
      }
    },
  };
}
