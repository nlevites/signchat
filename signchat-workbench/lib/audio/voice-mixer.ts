...
export function createVoiceMixer(options: VoiceMixerOptions = {}): VoiceMixer {
  ...
  const visibilityHandler = () => {
    if (document.visibilityState === "visible") {
      ...
      LogBus.info("audio", "audioCtx resumed on visibilitychange");
    } else {
      LogBus.debug("audio", "audioCtx suspended (tab hidden)");
    }
  };
  ...
  LogBus.debug("audio", "voice mixer created", {
    sampleRate,
  ...
  return {
    ...
    attachMic(stream: MediaStream): void {
      ...
      LogBus.debug("audio", "mic attached", {
        trackId: tracks[0]?.id,
      });
    },
    detachMic(): void {
      if (micSourceNode) {
        ...
        micSourceNode = null;
        LogBus.debug("audio", "mic detached");
      }
    },
    setMonitorEnabled(on: boolean): void {
      ...
      LogBus.debug("audio", "monitor toggled", { on });
    },
    scheduleTtsChunk(buffer: AudioBuffer): ScheduleResult {
    ...
    dispose(): void {
      ...
      LogBus.debug("audio", "voice mixer disposed");
    },
  };
}
